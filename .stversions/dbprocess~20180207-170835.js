  ...
  ipcMain.on('loadQuizes', (event, arg) => {
    ...
    if (overduearray.length > 0) {
      ...
      if (l > 0) {
        ...
        for (i = 0; i < l; i++) {
          if (overduearray[i]) {
            cme.find({ id: overduearray[i].id}, function(err, data) {
      		      if (err) console.log(err);
                if(data[i]) {
                  data.types[0] = 'q1';
                  var cmo = JSON.parse(arg.cmobject);
                  if (cmo['style']['object']['str']) {
                    cmo['style']['object']['str'] = String(overduearray[i].interval);
                    cmo['style']['object']['weight'] = overduearray[i].dif;
                    ...
                    if (i + 1 === l) {
                      event.sender.send('loadedQuizes', quizmes);
                    }
                  }
                }
      	     });
