...
export class CmosvgService {
  ...
  // creates a rectangle
  createRectangle(cme, bbox, cmg: any) {
    let s = Snap('#cmsvg');
    let mrg = 2;
  ...
  createRounded(cme, bbox, cmg: any) {
    let s = Snap('#cmsvg');
    let mrg = 5;
    let a = s.rect((bbox.x - (mrg / 2)), (bbox.y - (mrg / 2)), (bbox.w + mrg), (bbox.h + mrg), 3, 3);
    a.attr({
      fill: cme.cmobject.style.object.color0,
      stroke: cme.cmobject.style.object.color1,
      strokeWidth: cme.prio + 1,
  ...
  // creates an ellipse
  createEllipse(cme, bbox, cmg: any) {
    let s = Snap('#cmsvg');
    let mrg = 2;
  ...
  // test
  createTest(cme, bbox, cmg: any) {
    let s = Snap('#cmsvg');
    let mrg = 5;
    let a = s.rect((bbox.x - (mrg / 2)), (bbox.y - (mrg / 2)), (bbox.w + mrg), (bbox.h + mrg), 5, 5);
    a.attr({
      fill: cme.cmobject.style.object.color0,
      stroke: cme.cmobject.style.object.color1,
      strokeWidth: cme.prio + 1,
