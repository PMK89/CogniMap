...
export class ElementService {
  ...
  // sets current element1
  setSelectedElement1(id: number) {
    this.store.select('elements')
        .subscribe(data => {
            for (let key in data) {
              if (data[key]) {
                if (data[key].id === id) {
                  let action = {type: 'ADD_SCME', payload: data[key] };
                  this.store.dispatch(action);
                  this.currentElement1 = data[key];
                  id = 0;
                  console.log(this.currentElement1, '!');
                }
              }
            }
          },
  ...
  setInactive(id: number) {
    ...
    if (this.cmsettings.mode === 'progress') {
      ...
      this.settingsService.updateSettings(this.cmsettings);
      for (let i in this.currentElement.cmobject.links) {
        if (this.currentElement.cmobject.links[i]) {
          let link = this.currentElement.cmobject.links[i];
          this.changeLinkTitle(link.id, this.currentElement.title, link.start);
  ...
  // Set Element as draggable *shouldn't be used*
  setDrag(id: number) {
    let action = {type: 'DRAG_CME', payload: {id: id} };
    this.store.dispatch(action);
  }
  // Set Element as not draggable *shouldn't be used*
  setNoDrag(id: number) {
  ...
  moveElement(x, y) {
    if (this.currentElement && x && y) {
      this.currentElement.coor.x += x;
      this.currentElement.coor.y += y;
      if (this.currentElement.dragging === true) {
        this.currentElement.dragging = false;
        // console.log(this.currentElement);
        ...
        for (let i in this.currentElement.cmobject.links) {
          if (this.currentElement.cmobject.links[i]) {
            let link = this.currentElement.cmobject.links[i];
            this.changeLink(link.id, this.currentElement.coor.x, this.currentElement.coor.y, link.start);
            // console.log('move: ', link.id);
          }
        }
  ...
  changeLinkTitle(id: number, title: string, start: boolean) {
    this.store.select('elements')
        .subscribe(data => {
            for (let key in data) {
              if (data[key]) {
                if (data[key].id === id) {
                  if (start) {
                    data[key].title0 = title;
                  } else {
                    data[key].title1 = title;
                  }
                  this.updateElement(data[key]);
                  id = 0;
                }
              }
            }
  ...
  // changes link if object is moved
  changeLink(id: number, x: number, y: number, start: boolean) {
    this.store.select('elements')
        .subscribe(data => {
            for (let key in data) {
              if (data[key]) {
