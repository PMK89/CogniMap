...
export class ElementService {
  ...
  public CMEtoCMEol(cme: CME) {
    if (cme !== undefined && typeof cme['cmobject'] === 'string') {
      let cmeol = {
        id: cme.id,
        x0: cme.x0,
        y0: cme.y0,
  ...
  public setmode() {
    if (this.cmsettings) {
      ...
      if (this.cmsettings.mode === 'dragging') {
        ...
        console.log('cutting', this.selCMEo);
        /*
        if (this.selCMEo) {
          // console.log(this.selCMEo);
          this.selCMEo.state = 'cutting';
          this.updateSelCMEo(this.selCMEo);
        } else if (this.selCMEoArray.length > 0) {
          this.selectionGroup(this.selCMEoArray, this.selCMElArray);
        }*/
      ...
      if (['selecting', 'dragging', 'cutting'].indexOf(this.cmsettings.mode) === -1) {
  ...
  public moveElement(x, y) {
    ...
    if (this.selCMEo && x && y) {
      if (this.selCMEo.state === 'dragging' || this.selCMEo.state === 'cutting') {
        // console.log(x, y);
        ...
        this.selCMEo.prep = '';
        this.selCMEo.state = 'selected';
        console.log('move Element', this.selCMEo);
        this.cmsettings.mode = 'edit';
        this.settingsService.updateSettings(this.cmsettings);
        this.updateSelCMEo(this.selCMEo);
      ...
      if (this.allCME.length > 0) {
        ...
        console.log(dataArray.length);
        /*
        this.store.dispatch({type: 'ADD_CME_FROM_DB', payload: [] });
        for (let key in dataArray) {
          if (dataArray[key]) {
            this.store.dispatch({type: 'ADD_CME', payload: dataArray[key] });
            console.log(key);
          }
        }*/
      ...
      this.cmelements
          .subscribe((data) => {
              ...
              if (Link === undefined && Link === null) {
                let rawLink = this.getDBCMEbyId(id);
                console.log(rawLink, id);
                if (rawLink) {
                  Link = this.CMEtoCMEol(rawLink);
                }
              }
              if (Link !== undefined && Link === null) {
                console.log(Link);
                ...
                if (this.selCMEl) {
                  if (this.selCMEl.id === id) {
                    this.selCMEl.x0 = Link.x0;
                    this.selCMEl.y0 = Link.y0;
                    this.selCMEl.x1 = Link.x1;
                    this.selCMEl.y1 = Link.y1;
                    this.selCMEl.coor = Link.coor;
                    this.selCMEl.prep = '';
                  }
                }
                this.updateCME(Link);
